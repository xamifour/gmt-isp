...
class AbstractPlan(OrgMixin, BaseMixin, OrderedModel):
    ...
    radius_group = models.ForeignKey('openwisp_radius.RadiusGroup', on_delete=models.CASCADE,
        blank=True,
        null=True,
        verbose_name=_('RADIUS group:'), related_name="plan_radius_group",
        )
    temp_radius_group = models.ForeignKey('openwisp_radius.RadiusGroup', on_delete=models.CASCADE,
        blank=True,
        null=True,
        verbose_name=_('Temporary RADIUS group:'), related_name="plan_temp_radius_group",
        )
...
class AbstractUserPlan(BaseMixin):
    """
...
class AbstractPricing(OrgMixin, BaseMixin):
    """
...
class AbstractPlanPricing(BaseMixin):
...
class PlanBandwidthSettingsManager(models.Manager):
    def get_queryset(self):
        return (
            super().get_queryset().select_related("plan", "bandwidth")
        )


class AbstractPlanBandwidthSettings(BaseMixin, models.Model):
    plan = models.ForeignKey("gmtisp_billing.Plan", on_delete=models.CASCADE)
    bandwidth = models.ForeignKey("gmtisp_billing.BandwidthSettings", on_delete=models.CASCADE, blank=True, null=True)
    rate_limit_up = models.CharField(_('Rate limit up'), max_length=64, blank=True, null=True)
    rate_limit_down = models.CharField(_('Rate limit down'), max_length=64, blank=True, null=True)
    burst_rate_up = models.CharField(_('Burst rate up'), max_length=64, blank=True, null=True)
    burst_rate_down = models.CharField(_('Burst rate down'), max_length=64, blank=True, null=True)
    burst_threshold_up = models.CharField(_('Burst threshold up'), max_length=64, blank=True, null=True)
    burst_threshold_down = models.CharField(_('Burst threshold down'), max_length=64, blank=True, null=True)
    min_rate_up = models.CharField(_('Min rate up'), max_length=64, blank=True, null=True)
    min_rate_down = models.CharField(_('Min rate down'), max_length=64, blank=True, null=True)
    burst_time_up = models.CharField(_('Burst time up'), max_length=64, blank=True, null=True)
    burst_time_down = models.CharField(_('Burst time down'), max_length=64, blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.plan.name} {self.bandwidth.name}" 


class AbstractOrder(BaseMixin):
    """
