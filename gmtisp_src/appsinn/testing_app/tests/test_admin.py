...
class TestAdmin(AdminTestMixin, CreateMixin, TestCase):
    ...
    def test_radiusaccounting_change(self):
        ...
        response = self.client.get(
            reverse('admin:testing_app_radiusaccounting_change', args=[obj.pk])
        )
        self.assertContains(response, 'ok')
        self.assertNotContains(response, 'errors')

    def test_radiusaccounting_changelist(self):
        url = reverse('admin:testing_app_radiusaccounting_changelist')
    ...
    def test_alwayshaschangedmixin(self):
        ...
        url = reverse('admin:testing_app_project_add')
        ...
        change_url = reverse('admin:testing_app_project_change', args=[project.pk])
    ...
    def test_timereadonlyadminmixin(self):
        url = reverse('admin:testing_app_shelf_add')
        response = self.client.get(url)
    ...
    def test_uuid_field_in_change(self):
        ...
        path = reverse('admin:testing_app_project_change', args=[p.pk])
        response = self.client.get(path)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'field-uuid')

    def test_uuid_field_in_add(self):
        path = reverse('admin:testing_app_project_add')
        response = self.client.get(path)
        self.assertEqual(response.status_code, 200)
    ...
    def test_copyablefields_admin(self):
        ...
        path = reverse(
            'admin:testing_app_radiusaccounting_change', args=[radius_acc.pk]
        )
        self.assertEqual(
    ...
    def test_stacked_inline_help_text(self):
        ...
        path = reverse('admin:testing_app_project_change', args=[project.pk])
        response = self.client.get(path)
        self.assertContains(
    ...
    def test_filters(self):
        with self.subTest('Test when number of filters is less than equal to 4'):
            url = reverse('admin:testing_app_operator_changelist')
            response = self.client.get(url)
        ...
        with self.subTest('Test when number of filters is greater than 5'):
            url = reverse('admin:testing_app_shelf_changelist')
            response = self.client.get(url)
            real_count = self._assert_contains(
                response,
                'class="ow-filter',
    ...
    def test_ow_auto_filter_view(self):
        ...
        url = f'{url}?app_label=testing_app&model_name=shelf&field_name=book'
        user = User.objects.create(
            username='operator',
            password='pass',
            email='email@email',
    ...
    def test_ow_autocomplete_filter_uuid_exception(self):
        url = reverse('admin:testing_app_book_changelist')
    ...
    def test_organization_radius_settings_admin(self):
        ...
        url = reverse(
            'admin:testing_app_organizationradiussettings_change',
